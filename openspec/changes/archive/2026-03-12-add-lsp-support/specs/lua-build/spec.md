...
- **THEN** 编译成功完成，无错误
- **AND** 在 `build/` 目录下生成 `lua.exe`、`luac.exe`、`tluac.exe`、`tlua.exe` 和 `tlua-lsp.exe`
...
#### Scenario: tlua-lsp 构建目标

- **WHEN** CMakeLists.txt 定义 `tlua-lsp` 可执行文件目标
- **THEN** SHALL 包含 `tlua/tlua_lsp.c`、`tlua/tlua_json.c`、`tlua/tlua_lexer.c`、`tlua/tlua_parser.c` 源文件
- **AND** SHALL 配置 `tlua/` 为 include 路径
- **AND** 不需要链接 Lua 运行时库（LSP 服务器不嵌入 Lua VM）

#### Scenario: Install 目标包含 LSP 服务器

- **WHEN** 运行 `cmake --install build`
- **THEN** `tlua-lsp` 可执行文件 SHALL 被安装到目标 bin 目录
